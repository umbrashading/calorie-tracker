-- Per-user, per-day aggregation consumed by dashboard and history.
-- security_invoker ensures RLS on underlying tables is respected.
...
select
  ...
  round(coalesce(ds.steps, 0) * 0.0005 * coalesce(pb.weight_kg, 0))::integer as steps_calories,
  coalesce(bbd.exercise_calories, 0) as exercise_calories,
  (
    round(pb.bmr * pb.activity_multiplier)
    + round(coalesce(ds.steps, 0) * 0.0005 * coalesce(pb.weight_kg, 0))
