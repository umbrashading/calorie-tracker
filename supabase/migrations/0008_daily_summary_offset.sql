-- Steps calories are an offset from average daily steps, not additive on top of baseline.
-- Baseline already reflects typical activity via the activity_level multiplier.
-- This view is used for completed historical days (no time-of-day proration).
...
select
  ...
  round(pb.bmr * pb.activity_multiplier)::integer as baseline_calories,
  coalesce(ds.steps, pb.average_daily_steps) as steps,
  round(
    (coalesce(ds.steps, pb.average_daily_steps) - pb.average_daily_steps)
    * 0.0005
    * coalesce(pb.weight_kg, 0)
  )::integer as steps_calories,
  coalesce(bbd.exercise_calories, 0) as exercise_calories,
  (
    round(pb.bmr * pb.activity_multiplier)
    + round(
      (coalesce(ds.steps, pb.average_daily_steps) - pb.average_daily_steps)
      * 0.0005
      * coalesce(pb.weight_kg, 0)
